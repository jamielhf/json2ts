...
function handleArrayExpression(name, node) {
  ...
  node.elements.forEach(function (item) {
    if (t__namespace.isLiteral(item)) {
      ...
      nameMap.set(tsName, t__namespace.tsTypeReference(t__namespace.identifier(tsName)));
      var arr = handleObjectExpression(item);
      var tNode = t__namespace.exportNamedDeclaration(t__namespace.tsInterfaceDeclaration(t__namespace.identifier(tsName), null, null, t__namespace.tsInterfaceBody(arr)));
    ...
    } else if (t__namespace.isArrayExpression(item)) {
  ...
  if (typeArr.length === 0) {
    typeArr.push(t__namespace.tsUnknownKeyword());
  }

  return t__namespace.tsArrayType(typeArr.length > 1 ? t__namespace.tsUnionType(typeArr) : typeArr[0]);
}
...
function transform(ast, name) {
  ...
  var program = t__namespace.program([t__namespace.exportNamedDeclaration(t__namespace.tsModuleDeclaration(t__namespace.identifier(name), t__namespace.tSModuleBlock(contentArr)))]);
